...
Clarinet.test({
    name: "Can record energy and claim earnings",
    async fn(chain: Chain, accounts: Map<string, Account>) {
        const deployer = accounts.get('deployer')!;
        const wallet1 = accounts.get('wallet_1')!;
        ...
        let block = chain.mineBlock([
            ...
            Tx.contractCall('solar-panel-shares', 'buy-shares', [
                types.uint(0),
                types.uint(50)  // 50% ownership
            ], wallet1.address),
            // Record energy
            Tx.contractCall('solar-panel-shares', 'record-energy', [
                types.uint(0),
                types.uint(10000)  // 10kWh
            ], deployer.address),
            // Claim earnings
            Tx.contractCall('solar-panel-shares', 'claim-earnings', [
                types.uint(0)
            ], wallet1.address)
        ]);
        ...
        // Verify earnings were transferred
        let earnings = chain.mineBlock([
            Tx.contractCall('solar-panel-shares', 'get-investor-earnings', [
                types.principal(wallet1.address)
            ], deployer.address)
        ]);
        
        // Should have earned 50% of earnings for 10kWh
        assertEquals(earnings.receipts[0].result, types.uint(500000)); // $0.50 worth of STX
    }
});
